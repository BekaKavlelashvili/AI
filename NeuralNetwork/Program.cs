public class NeuralNetwork
{
    ...
    private double[,] Activate(double[,] matrix, bool isDerivative)
    {
        ...
        for (int row = 0; row < numberOfRows; row++)
        {
            for (int column = 0; column < numberOfColumns; column++)
            {
                ...
                double derivativeSigmoidOutput = result[row, column] = matrix[row, column] * (1 - result[row, column]);
    ...
    public void Train(double[,] trainingInputs, double[,] trainingOutputs, int numberOfIterations)
    {
        for (int iteration = 0; iteration < numberOfIterations; iteration++)
        {
            ...
            double[,] error = PerformeOperation(trainingInputs, output, OPERATION.Substruct);
            double[,] adjustment = DotProduct(Transpose(trainingInputs), PerformeOperation(error, Activate(output, true), OPERATION.Multiply));
            weights = PerformeOperation(weights, adjustment, OPERATION.Add);
        }
    }
    ...
    private double[,] PerformeOperation(double[,] matrix1, double[,] matrix2, OPERATION operation)
    {
    ...
    static void Main(string[] args)
    {
        ...
        double[,] outputs = neuralNetwork.Think(new double[,] { { 0, 1, 0 }, { 0, 0, 0 }, { 0, 0, 1 } });
        PrintMatrix(outputs);
    }
